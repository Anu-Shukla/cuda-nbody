...
|---------------|-------|----------------|----------|
| CPU (OpenMP)  | 4096  | 3.3            | ~102     |
| CPU (OpenMP)  | 8192  | 10.3           | ~131     |
| CPU (OpenMP)  | 16384 | 37.4           | ~143     |
| Naive (GPU)   | 4096  | 0.251          | ~1,339   |
| Naive (GPU)   | 8192  | 0.508          | ~2,643   |
| Naive (GPU)   | 16384 | 1.027          | ~5,227   |
| Tiled (GPU)   | 4096  | 0.193          | ~1,739   |
| Tiled (GPU)   | 8192  | 0.383          | ~3,500   |
| Tiled (GPU)   | 16384 | 0.765          | ~7,017   |
| Float4 (GPU)  | 4096  | 0.195          | ~1,719   |
| Float4 (GPU)  | 8192  | 0.388          | ~3,457   |
| Float4 (GPU)  | 16384 | 0.775          | ~6,931   |

All results use `-O3 --use_fast_math` compilation flags. CPU results use `-O3 -fopenmp` flags on Apple M4 Pro (12 cores).
Peak RTX 4090 is around 82 TFLOPs.

## GPU vs CPU Speedup (Tiled vs OpenMP)

| N     | CPU (ms) | GPU Tiled (ms) | Speedup |
|-------|----------|----------------|---------|
| 4096  | 3.3      | 0.193          | ~17x    |
| 8192  | 10.3     | 0.383          | ~27x    |
| 16384 | 37.4     | 0.765          | ~49x    |

Speedup grows with N as the GPU becomes better utilized at larger problem sizes.

## Implementations

### 1. CPU Reference (`cpu_reference.cpp`)
Parallelized across 12 cores using OpenMP. Same force calculation as GPU naive kernel, used as baseline for speedup comparison.

### 2. Naive Kernel (`naive.cu`)
...
### 3. Shared Memory Tiling (`tiled.cu`)
...
### 4. Float4 Vectorized Loads (`float4.cu`)
